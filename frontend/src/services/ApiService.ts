...
export class ApiService {
  ...
  async getNonce(address: string): Promise<{ message: string; timestamp: number; nonce: string }> {
    console.log('🔍 Getting nonce for address:', address);
    const result = await this.request(`/auth/nonce/${address}`);
    console.log('🔍 Nonce API response:', result);
    return result;
  }
  ...
  async updateUserProfile(address: string, data: any): Promise<any> {
    return this.request(`/users/${address}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
